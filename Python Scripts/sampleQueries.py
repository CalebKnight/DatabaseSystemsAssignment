...
# -- Which actors or directors won an award at the Oscars?
SelectTable("SELECT People.first_name, People.last_name, Award.name as 'Won Award For' FROM People JOIN Award_Winner ON Award_Winner.people_id = People.people_id JOIN Award ON Award.award_id = Award_Winner.award_id LIMIT 20;")

# -- Which actors won the most awards?
SelectTable("SELECT People.first_name, People.last_name, COUNT(Award.name) as 'Number of Awards Won' FROM People JOIN Award_Winner ON Award_Winner.people_id = People.people_id JOIN Award ON Award.award_id = Award_Winner.award_id GROUP BY People.people_id ORDER BY COUNT(Award.name) DESC LIMIT 10;")

# -- Which films won the most awards?
SelectTable("SELECT Film.title as 'Film Title', COUNT(Award.name) as 'Number of Awards Won' FROM Film JOIN Award_Winner ON Award_Winner.film_id = Film.film_id JOIN Award ON Award.award_id = Award_Winner.award_id GROUP BY Film.film_id ORDER BY COUNT(Award.name) DESC LIMIT 10;")

# -- Which films were the longest?
SelectTable("SELECT Film.title as 'Film Title', Film.length as 'Film Length' FROM Film ORDER BY Film.length DESC LIMIT 10;")

# -- Which films were the shortest?
SelectTable("SELECT Film.title as "Film Title", Film.length as "Film Length" FROM Film ORDER BY Film.length ASC LIMIT 10;")
...
SelectTable("SELECT Film.title as 'Film Title', Film.year as 'Year', COUNT(Award.name) as 'Number of Awards Won' FROM Film JOIN Award_Winner ON Award_Winner.film_id = Film.film_id JOIN Award ON Award.award_id = Award_Winner.award_id WHERE Film.year BETWEEN 2000 AND 2009 GROUP BY Film.film_id ORDER BY COUNT(Award.name) DESC LIMIT 10;")
